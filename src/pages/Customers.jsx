...
import {
    ...
    FaUsers, FaBuilding, FaUser, FaToggleOn, FaToggleOff,
...
import {
    ...
    ViewCustomerModal,
    DeleteCustomerModal,
} from '../components/modals/CustomerModals';
...
const StatCard = ({ label, value, sub, icon, color }) => {
    const colorMap = {
        ...
        violet: { wrap: 'bg-violet-50 border-violet-200', icon: 'text-violet-600' },
    ...
    return (
      ...
      border-t border-gray-100 flex-wrap gap-3">
            <span className="text-sm text-gray-500 font-medium">
                ...
                {' of '}
                <strong className="text-gray-900">{total}</strong> customers
            </span>
            ...
            <div className="flex items-center gap-1">
                ...
                <button onClick={() => onChange(page + 1)}
                    ...
                    className={navBtn(page === Math.ceil(total / perPage))}>
...
const Customers = () => {
    ...
    // ── Date filter applied on top of hook's filtered list ────────────────────
    const dateFiltered = customers.filter(c => {
        if (!dateFrom && !dateTo) return true;
        const since = c.customerSince ? new Date(c.customerSince) : null;
        if (!since) return false;
        if (dateFrom && since < new Date(dateFrom)) return false;
        if (dateTo && since > new Date(dateTo)) return false;
        return true;
    });
    ...
    return (
        <Layout>
            <div className="fade-up max-w-[1400px] mx-auto flex flex-col gap-6">
              ...
              transition-all duration-200 cursor-pointer border-none">
                        <FaPlus className="text-[11px]" /> Add Customer
                    </button>
                </div>
                ...
                {/* ── Quick Tabs — All / Individual / Business only ── */}
                    ...
                    {[
                        {
                            ...
                            onClick: () => { handleType('All'); handleStatus('All'); },
                        },
                        {
                            label: 'Individual',
                ...
                ${tab.active
                    ...
                    <span className="ml-auto text-sm text-gray-500 font-medium">
                        Showing{' '}
                        <strong className="text-gray-900">{dateFiltered.length}</strong>
                        {' '}of{' '}
                        <strong className="text-gray-900">{stats.total}</strong> customers
                    </span>
                </div>
      ...
      transition-all duration-200 cursor-pointer">
                  ...
                  text-gray-300 text-xs pointer-events-none" />
                        ...
                        {/* Customer Type */}
                        <FilterInput label="Customer Type">
                            <select
                                value={filterType}
                        ...
                        {/* Date From */}
                        <FilterInput label="Customer Since (From)">
                            <input
                                type="date"
                                value={dateFrom}
                        ...
                        {/* Date To */}
                        <FilterInput label="Customer Since (To)">
                            <input
                                type="date"
                                value={dateTo}
            ...
            flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-xs text-gray-500 mt-0.5 font-medium">
                                {dateFiltered.length}{' '}
                                {dateFiltered.length === 1 ? 'customer' : 'customers'} found
                            </p>
                        </div>
                    </div>
                        ...
                        <table className="w-full border-collapse text-sm">
                            <thead>
                                <tr className="bg-gray-50 border-b border-gray-100">
                                    {['Customer', 'Mobile', 'Email', 'Type', 'Status', 'Since', 'Actions'].map(h => (
                                        <th key={h}
                        ...
                        uppercase tracking-wider whitespace-nowrap text-left">
                            ...
                            <tbody>
                              ...
                              flex items-center justify-center">
                                                            <FaUsers className="text-pink-300 text-2xl" />
                                                        </div>
